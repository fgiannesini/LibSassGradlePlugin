...
public class LibSassCompilerTest {
    ...
    public void initPluginParameterProvider() throws IOException {
        Mockito.when(this.parametersProvider.getIncludePaths()).thenReturn(
                new String[] { this.folder.newFile().getAbsolutePath() });
        Mockito.when(this.parametersProvider.getInputSyntax())
                .thenReturn(PluginInputSyntax.SCSS);
        Mockito.when(this.parametersProvider.getOmitSourceMappingUrl())
                .thenReturn(Boolean.FALSE);
        Mockito.when(this.parametersProvider.getOutputStyle())
                .thenReturn(PluginOutputStyle.NESTED);
        Mockito.when(this.parametersProvider.getPrecision())
                .thenReturn(Integer.valueOf(5));
        Mockito.when(this.parametersProvider.getSourceComments())
                .thenReturn(Boolean.FALSE);
        Mockito.when(this.parametersProvider.getSourceMapContents())
                .thenReturn(Boolean.FALSE);
        Mockito.when(this.parametersProvider.getSourceMapEmbed())
                .thenReturn(Boolean.FALSE);

        Mockito.when(this.parametersProvider.getOutputUri())
                .thenReturn(this.folder.newFile().toURI());
        Mockito.when(this.parametersProvider.getInputUri())
                .thenReturn(this.folder.newFile().toURI());
    }
    ...
    public void generateSourceMap_Generation() throws IOException {
        ...
        Mockito.when(this.parametersProvider.getSourceMapUri())
                .thenReturn(sourceMapFile.toURI());
        ...
        // Check
        Assert.assertTrue(StringUtils.isNotBlank(FileUtils
                .readFileToString(sourceMapFile, Charset.defaultCharset())));
        Mockito.verify(this.logger, Mockito.never()).error(Mockito.anyString(),
    ...
    public void generateSourceMap_No_SourceMap() throws IOException {
        ...
        // Check
        Assert.assertTrue(StringUtils.isBlank(FileUtils
                .readFileToString(sourceMapFile, Charset.defaultCharset())));
        Mockito.verify(this.logger, Mockito.never()).error(Mockito.anyString(),
                Mockito.any(IOException.class));
    }
    ...
    public void writeCss() throws IOException {
        ...
        // Check
        Assert.assertTrue(StringUtils.isNotBlank(FileUtils
                .readFileToString(outputFile, Charset.defaultCharset())));
        Mockito.verify(this.logger, Mockito.never()).error(Mockito.anyString(),
                Mockito.any(IOException.class));
    }
