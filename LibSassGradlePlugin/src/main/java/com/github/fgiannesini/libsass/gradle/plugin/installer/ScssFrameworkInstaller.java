...
public class ScssFrameworkInstaller {
	 ...
	 * @throws Exception
	 */
	public void installFramework(final FrameworkPropertiesEnum frameworkProperties) throws Exception {
		final File folder = this.downloadSources(frameworkProperties);
		this.installSources(frameworkProperties, folder);
	}

	private void installSources(final FrameworkPropertiesEnum frameworkProperties, final File folder) throws Exception {
		try {
			final File sourcesFolder = this.getSourcesFolder(frameworkProperties, folder);

			final Path installationPath = this.getInstallationPath(frameworkProperties.getFolderInstallationName());
	...
	private File downloadSources(final FrameworkPropertiesEnum frameworkProperties) throws IOException {
		final JRubyCaller jrubyCaller = new JRubyCaller(this.logger);
		final File folder = jrubyCaller.unpack(frameworkProperties.getGemName(), this.versionToDownload);
		return folder;
	}
	...
	protected void copySourcesToInstallationPath(final File sourcesFolder, final Path installationPath)
		...
		for (final File source : listFiles) {
			if (source.isDirectory()) {
				FileUtils.copyDirectory(source, installationPath.resolve(source.getName()).toFile());
				this.logger.info("Folder " + source.getName() + " copied");
			} else {
				FileUtils.copyFile(source, installationPath.resolve(source.getName()).toFile());
	 ...
	 * @throws IOException
	 */
	protected Path getInstallationPath(final String folderInstallationName) throws IOException {
		if (this.installationPath == null) {
			throw new IOException("Installation path is a mandatory parameter, please set it");
		}

		final Path installPath = Paths.get(this.project.getRootDir().getAbsolutePath(),
				this.installationPath.toString(), folderInstallationName);
	 ...
	 * @throws Exception
	 */
	protected File getSourcesFolder(final FrameworkPropertiesEnum frameworkProperties, final File folder)
			throws Exception {
		final File unpackedSourceFolder = this.getAndCheckUnpackedSourceFolder(frameworkProperties.getGemName(),
				folder);
		final File sourcesFolder = Paths.get(unpackedSourceFolder.getAbsolutePath())
				.resolve(frameworkProperties.getInternalGemSourcesPath()).toFile();
		this.logger.info(frameworkProperties + " sources folder : " + sourcesFolder.getAbsolutePath());
	 ...
	 * @throws Exception
	 */
	protected File getAndCheckUnpackedSourceFolder(final String gemName, final File folder) throws Exception {
		...
		if (unpackedFiles.length != 1) {
			this.logger
					.error("Folder " + folder.getAbsolutePath() + "should contains only one folder. Folder content :");
			Arrays.stream(unpackedFiles).map(File::getName).forEach(this.logger::error);
			throw new Exception("A problem occured while retrieving " + gemName + " sources files");
		}
